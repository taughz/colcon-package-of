...
Find packages for the given files:
  ...
  $ colcon package-of -s src/pkg1/package.xml
  pkg1\tsrc/pkg1\t(ros.ament_python) (esc)

  $ colcon package-of --strict src/pkg3/test/test_pep257.py
  pkg3\tsrc/pkg3\t(ros.ament_python) (esc)

Try finding something not belonging to a package:

  $ colcon package-of -s src/pkgX/this/does/not/exist
  Unmatched file path: src/pkgX/this/does/not/exist
  [1]

Find multiple packages:

  $ colcon package-of -s src/pkg1/package.xml src/pkg2/pkg2/lib2/__init__.py
  pkg1\tsrc/pkg1\t(ros.ament_python) (esc)
  pkg2\tsrc/pkg2\t(ros.ament_python) (esc)

  $ colcon package-of -s src/pkg3/test/test_pep257.py src/pkgX/this/does/not/exist
  Unmatched file path: src/pkgX/this/does/not/exist
  [1]

Print only package names:

  $ colcon package-of -s -n src/pkg1/package.xml
  pkg1

Print only package paths:

  $ colcon package-of -s -p src/pkg1/package.xml
  src/pkg1
